...
app.get("/api/scrape-product", async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      return res.status(403).json({ error: "Amazon blocked the request with Captcha" });
    }
    ...
    if (results.length === 0) {
      return res.status(204).json({ message: "No products found. Amazon may have changed the structure or blocked the request." });
    }
